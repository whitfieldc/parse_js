    ...
    class Function < Base
      ...
      def js_call(scope_chain, *params)
        arguments.each_with_index { |name, i|
          scope_chain[name.value] = params[i] || ParseJS::Runtime::UNDEFINED
        }
        function_visitor  = ParseJS::Visitors::FunctionVisitor.new(scope_chain)
        eval_visitor      = ParseJS::Visitors::EvaluationVisitor.new(scope_chain)
